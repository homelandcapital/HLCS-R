...
import type { CommunityProjectInterest, CommunityProjectInterestStatus, AuthenticatedUser, CommunityProjectInterestMessage, PlatformAdmin, UserRole } from '@/lib/types';
...
type CommunityInterestWithConversation = CommunityProjectInterest & { conversation: CommunityProjectInterestMessage[] };
...
export default function ProjectInterestsManagementPage() {
  ...
  const [selectedInterest, setSelectedInterest] = useState<CommunityInterestWithConversation | null>(null);
  ...
  const handleViewDetails = async (interest: CommunityProjectInterest) => {
    ...
    setReplyMessage('');
    try {
      const fullInterestData = await fetchInterestWithConversation(interest.id, 'community');
      setSelectedInterest(fullInterestData as CommunityInterestWithConversation);
    } catch (error: any) {
      toast({ title: "Error", description: `Could not fetch conversation: ${error.message}`, variant: "destructive" });
      setSelectedInterest({ ...interest, conversation: [] }); // Set with empty conversation on error
    } finally {
  ...
  const handleAdminReply = () => {
    ...
    startReplyTransition(async () => {
      const result = await addAdminReplyToInterest(selectedInterest.id, 'community', user as PlatformAdmin, replyMessage);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-headline flex items-center"><FileHeart className="mr-3 h-8 w-8 text-primary" /> Community Project Interests</h1>
        ...
        <CardHeader>
          ...
          <div className="pt-4 flex flex-col sm:flex-row gap-4">
            ...
            <Select value={statusFilter} onValueChange={(value) => setStatusFilter(value as CommunityProjectInterestStatus | 'all')}>
              ...
              <SelectContent><SelectItem value="all">All Statuses</SelectItem>{communityProjectInterestStatuses.map(status => (<SelectItem key={status} value={status} className="capitalize">{status}</SelectItem>))}</SelectContent>
            </Select>
          </div>
        </CardHeader>
        <CardContent>
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow><TableHead>User</TableHead><TableHead>Original Project</TableHead><TableHead>Desired Location</TableHead><TableHead>Budget Tier</TableHead><TableHead>Date</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
              <TableBody>
                {filteredInterests.map((interest) => (
                  <TableRow key={interest.id}>
                    ...
                    <TableCell>{interest.project_id && interest.project_title ? (<Link href={`/community-projects/${interest.project_id}`} target="_blank" rel="noopener noreferrer" title={`View ${interest.project_title}`} className="font-medium text-primary hover:underline">{interest.project_title} <ExternalLink className="inline h-3 w-3 ml-0.5" /></Link>) : (<span className="text-xs text-muted-foreground">General Interest</span>)}</TableCell>
                    <TableCell><div className="text-sm">{interest.location_type === 'stateCapital' ? interest.state_capital : interest.lga_name}</div><div className="text-xs text-muted-foreground capitalize">{interest.location_type === 'stateCapital' ? 'State Capital' : 'LGA'}</div></TableCell>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader><DialogTitle className="font-headline text-xl flex items-center"><FileHeart className="mr-2 h-6 w-6 text-primary" /> Interest Details</DialogTitle><DialogDescription>Submitted on: {selectedInterest ? format(new Date(selectedInterest.created_at), "PPP p") : '...'}</DialogDescription></DialogHeader>
