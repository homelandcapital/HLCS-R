...
export default function MyListingsPage() {
  ...
  const platformSettings = mockPlatformSettings; // Still using mock settings for promotion tiers
  ...
  const handleConfirmDelete = async () => {
    ...
    if (error) {
      ...
      fetchAgentProperties((user as Agent).id); // Re-fetch properties
    }
  ...
  const handleConfirmPromotion = async () => {
    ...
    if (error) {
      ...
      fetchAgentProperties((user as Agent).id); // Re-fetch
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button asChild> <Link href="/agents/dashboard/add-property"> <span><PlusCircle className="mr-2 h-5 w-5" /> Add New Listing</span> </Link> </Button>
      </div>
          ...
          {agentProperties.map(property => (
            <Card key={property.id} className="flex flex-col shadow-lg hover:shadow-xl transition-shadow">
              ...
              <CardFooter className="grid grid-cols-3 gap-2 border-t pt-4">
                <Button variant="outline" size="sm" asChild title="View Listing" disabled={property.status !== 'approved'}> <Link href={`/properties/${property.id}`} target="_blank" rel="noopener noreferrer"> <span><Eye className="h-4 w-4" /></span> </Link> </Button>
