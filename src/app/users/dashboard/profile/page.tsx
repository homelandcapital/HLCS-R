...
// src/app/users/dashboard/profile/page.tsx
'use client';
...
import { UserCircle, Mail, Edit2, Lock, Shield } from 'lucide-react';
  ...
  if (!user || user.role !== 'user') {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">This page is for registered users only.</p>
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <div className="flex flex-col sm:flex-row items-center space-x-0 sm:space-x-6 space-y-4 sm:space-y-0">
            ...
            <div>
              <CardTitle className="text-3xl font-headline">{generalUser.name}</CardTitle>
              <CardDescription className="text-lg">Homeland Capital User</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-6 pt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InfoItem icon={<Mail />} label="Email Address" value={generalUser.email} />
            <InfoItem icon={<Shield />} label="Account Type" value="General User" />
          </div>
        </CardContent>
      </Card>
