...
export default function SavedPropertiesPage() {
  ...
  if (!user || user.role !== 'user') {
     return ( <div className="text-center py-12"> <h1 className="text-2xl font-headline">Access Denied</h1> <p className="text-muted-foreground">You need to be logged in as a user to view this page.</p> <Button asChild className="mt-4"> <Link href="/">Go to Homepage</Link> </Button> </div> );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {savedPropertiesDetails.length === 0 ? (
        <Card className="text-center py-12 shadow-lg"> <CardHeader> <SearchX className="mx-auto h-16 w-16 text-muted-foreground mb-4" /> <CardTitle className="font-headline text-2xl">No Saved Properties Yet</CardTitle> <CardDescription>You haven&apos;t saved any properties. Start exploring and save your favorites!</CardDescription> </CardHeader> <CardContent> <Button asChild size="lg"> <Link href="/properties">Find Properties</Link> </Button> </CardContent> </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
